---
title: GitHub 趋势 2026年1月
date: 2026-01-30T20:13:32Z
categories:
- monthly
keywords:
- git trending
- git 热门
- git 趋势
- git 排行榜
---
<link rel="stylesheet" href="/public/css/trending.css">
{{< raw >}}
	<main class="container">
        <div class="repo-list" id="repoList">

	
			<div class="repo-card">
				<p><a href="https://github.com/openclaw/openclaw" target="_blank">
    


      
        openclaw /

      openclaw</a></p>
				<p>
      Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 117299</span>
						<span>🔱 16551</span>
					</div>
				<div class="stars-today">⭐ 96808 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/anomalyco/opencode" target="_blank">
    


      
        anomalyco /

      opencode</a></p>
				<p>
      The open source coding agent.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 93176</span>
						<span>🔱 8628</span>
					</div>
				<div class="stars-today">⭐ 48778 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/obra/superpowers" target="_blank">
    


      
        obra /

      superpowers</a></p>
				<p>
      An agentic skills framework & software development methodology that works.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Shell</span>
						<span>⭐ 40530</span>
						<span>🔱 3074</span>
					</div>
				<div class="stars-today">⭐ 27222 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/anthropics/skills" target="_blank">
    


      
        anthropics /

      skills</a></p>
				<p>
      Public repository for Agent Skills
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Python</span>
						<span>⭐ 58895</span>
						<span>🔱 5757</span>
					</div>
				<div class="stars-today">⭐ 29129 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/anthropics/claude-plugins-official" target="_blank">
    


      
        anthropics /

      claude-plugins-official</a></p>
				<p>
      Official, Anthropic-managed directory of high quality Claude Code Plugins.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Shell</span>
						<span>⭐ 5509</span>
						<span>🔱 565</span>
					</div>
				<div class="stars-today">⭐ 4281 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/lbjlaq/Antigravity-Manager" target="_blank">
    


      
        lbjlaq /

      Antigravity-Manager</a></p>
				<p>
      Professional Antigravity Account Manager & Switcher. One-click seamless account switching for Antigravity Tools. Built with Tauri v2 + React (Rust).专业的 Antigravity 账号管理与切换工具。为 Antigravity 提供一键无缝账号切换功能。
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Rust</span>
						<span>⭐ 19747</span>
						<span>🔱 2269</span>
					</div>
				<div class="stars-today">⭐ 14472 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/anthropics/claude-code" target="_blank">
    


      
        anthropics /

      claude-code</a></p>
				<p>
      Claude Code is an agentic coding tool that lives in your terminal, understands your codebase, and helps you code faster by executing routine tasks, explaining complex code, and handling git workflows - all through natural language commands.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Shell</span>
						<span>⭐ 62483</span>
						<span>🔱 4675</span>
					</div>
				<div class="stars-today">⭐ 12887 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/remotion-dev/remotion" target="_blank">
    


      
        remotion-dev /

      remotion</a></p>
				<p>
      🎥 Make videos programmatically with React
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 33817</span>
						<span>🔱 2035</span>
					</div>
				<div class="stars-today">⭐ 8496 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/VectifyAI/PageIndex" target="_blank">
    


      
        VectifyAI /

      PageIndex</a></p>
				<p>
      📑 PageIndex: Document Index for Vectorless, Reasoning-based RAG
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Python</span>
						<span>⭐ 10815</span>
						<span>🔱 772</span>
					</div>
				<div class="stars-today">⭐ 6153 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/BloopAI/vibe-kanban" target="_blank">
    


      
        BloopAI /

      vibe-kanban</a></p>
				<p>
      Get 10X more out of Claude Code, Codex or any coding agent
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Rust</span>
						<span>⭐ 19833</span>
						<span>🔱 1850</span>
					</div>
				<div class="stars-today">⭐ 12024 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/Lissy93/web-check" target="_blank">
    


      
        Lissy93 /

      web-check</a></p>
				<p>
      🕵️‍♂️ All-in-one OSINT tool for analysing any website
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 31448</span>
						<span>🔱 2467</span>
					</div>
				<div class="stars-today">⭐ 4343 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/usememos/memos" target="_blank">
    


      
        usememos /

      memos</a></p>
				<p>
      An open-source, self-hosted note-taking service. Your thoughts, your data, your control — no tracking, no ads, no subscription fees.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Go</span>
						<span>⭐ 56251</span>
						<span>🔱 4028</span>
					</div>
				<div class="stars-today">⭐ 8771 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/bytedance/UI-TARS-desktop" target="_blank">
    


      
        bytedance /

      UI-TARS-desktop</a></p>
				<p>
      The Open-Source Multimodal AI Agent Stack: Connecting Cutting-Edge AI Models and Agent Infra
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 25037</span>
						<span>🔱 2415</span>
					</div>
				<div class="stars-today">⭐ 4899 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/shareAI-lab/learn-claude-code" target="_blank">
    


      
        shareAI-lab /

      learn-claude-code</a></p>
				<p>
      Bash is all you need！write a claude code with only 16 line code 
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Python</span>
						<span>⭐ 15881</span>
						<span>🔱 3488</span>
					</div>
				<div class="stars-today">⭐ 4053 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/steveyegge/beads" target="_blank">
    


      
        steveyegge /

      beads</a></p>
				<p>
      Beads - A memory upgrade for your coding agent
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Go</span>
						<span>⭐ 13744</span>
						<span>🔱 826</span>
					</div>
				<div class="stars-today">⭐ 6964 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/virattt/dexter" target="_blank">
    


      
        virattt /

      dexter</a></p>
				<p>
      An autonomous agent for deep financial research
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 9394</span>
						<span>🔱 1151</span>
					</div>
				<div class="stars-today">⭐ 4974 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/ComposioHQ/awesome-claude-skills" target="_blank">
    


      
        ComposioHQ /

      awesome-claude-skills</a></p>
				<p>
      A curated list of awesome Claude Skills, resources, and tools for customizing Claude AI workflows
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Python</span>
						<span>⭐ 28299</span>
						<span>🔱 2734</span>
					</div>
				<div class="stars-today">⭐ 15383 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/DataTalksClub/data-engineering-zoomcamp" target="_blank">
    


      
        DataTalksClub /

      data-engineering-zoomcamp</a></p>
				<p>
      Data Engineering Zoomcamp is a free 9-week course on building production-ready data pipelines. The next cohort starts in January 2026. Join the course here 👇🏼
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Jupyter Notebook</span>
						<span>⭐ 37639</span>
						<span>🔱 7622</span>
					</div>
				<div class="stars-today">⭐ 3602 stars this month</div>
				</div>
			</div>
	

		</div>
    </main>
{{< /raw >}}
