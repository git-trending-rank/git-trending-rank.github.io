---
title: GitHub 趋势 2026年1月
date: 2026-01-31T23:59:15Z
categories:
- monthly
keywords:
- git trending
- git 热门
- git 趋势
- git 排行榜
---
<link rel="stylesheet" href="/public/css/trending.css">
{{< raw >}}
	<main class="container">
        <div class="repo-list" id="repoList">

	
			<div class="repo-card">
				<p><a href="https://github.com/openclaw/openclaw" target="_blank">
    


      
        openclaw /

      openclaw</a></p>
				<p>
      Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 132304</span>
						<span>🔱 19139</span>
					</div>
				<div class="stars-today">⭐ 110784 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/anomalyco/opencode" target="_blank">
    


      
        anomalyco /

      opencode</a></p>
				<p>
      The open source coding agent.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 93960</span>
						<span>🔱 8742</span>
					</div>
				<div class="stars-today">⭐ 49296 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/obra/superpowers" target="_blank">
    


      
        obra /

      superpowers</a></p>
				<p>
      An agentic skills framework & software development methodology that works.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Shell</span>
						<span>⭐ 41057</span>
						<span>🔱 3121</span>
					</div>
				<div class="stars-today">⭐ 27667 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/anthropics/skills" target="_blank">
    


      
        anthropics /

      skills</a></p>
				<p>
      Public repository for Agent Skills
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Python</span>
						<span>⭐ 59623</span>
						<span>🔱 5831</span>
					</div>
				<div class="stars-today">⭐ 29149 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/anthropics/claude-plugins-official" target="_blank">
    


      
        anthropics /

      claude-plugins-official</a></p>
				<p>
      Official, Anthropic-managed directory of high quality Claude Code Plugins.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Shell</span>
						<span>⭐ 5851</span>
						<span>🔱 577</span>
					</div>
				<div class="stars-today">⭐ 4359 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/lbjlaq/Antigravity-Manager" target="_blank">
    


      
        lbjlaq /

      Antigravity-Manager</a></p>
				<p>
      Professional Antigravity Account Manager & Switcher. One-click seamless account switching for Antigravity Tools. Built with Tauri v2 + React (Rust).专业的 Antigravity 账号管理与切换工具。为 Antigravity 提供一键无缝账号切换功能。
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Rust</span>
						<span>⭐ 20005</span>
						<span>🔱 2290</span>
					</div>
				<div class="stars-today">⭐ 14270 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/anthropics/claude-code" target="_blank">
    


      
        anthropics /

      claude-code</a></p>
				<p>
      Claude Code is an agentic coding tool that lives in your terminal, understands your codebase, and helps you code faster by executing routine tasks, explaining complex code, and handling git workflows - all through natural language commands.
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Shell</span>
						<span>⭐ 62774</span>
						<span>🔱 4709</span>
					</div>
				<div class="stars-today">⭐ 12953 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/VectifyAI/PageIndex" target="_blank">
    


      
        VectifyAI /

      PageIndex</a></p>
				<p>
      📑 PageIndex: Document Index for Vectorless, Reasoning-based RAG
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Python</span>
						<span>⭐ 11170</span>
						<span>🔱 795</span>
					</div>
				<div class="stars-today">⭐ 6335 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/remotion-dev/remotion" target="_blank">
    


      
        remotion-dev /

      remotion</a></p>
				<p>
      🎥 Make videos programmatically with React
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 34098</span>
						<span>🔱 2056</span>
					</div>
				<div class="stars-today">⭐ 8733 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/Lissy93/web-check" target="_blank">
    


      
        Lissy93 /

      web-check</a></p>
				<p>
      🕵️‍♂️ All-in-one OSINT tool for analysing any website
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 31528</span>
						<span>🔱 2479</span>
					</div>
				<div class="stars-today">⭐ 4402 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/asgeirtj/system_prompts_leaks" target="_blank">
    


      
        asgeirtj /

      system_prompts_leaks</a></p>
				<p>
      Collection of extracted System Prompts from popular chatbots like ChatGPT, Claude & Gemini
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 JavaScript</span>
						<span>⭐ 29049</span>
						<span>🔱 4689</span>
					</div>
				<div class="stars-today">⭐ 3891 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/bytedance/UI-TARS-desktop" target="_blank">
    


      
        bytedance /

      UI-TARS-desktop</a></p>
				<p>
      The Open-Source Multimodal AI Agent Stack: Connecting Cutting-Edge AI Models and Agent Infra
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 25093</span>
						<span>🔱 2415</span>
					</div>
				<div class="stars-today">⭐ 4830 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/virattt/dexter" target="_blank">
    


      
        virattt /

      dexter</a></p>
				<p>
      An autonomous agent for deep financial research
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 9461</span>
						<span>🔱 1155</span>
					</div>
				<div class="stars-today">⭐ 5000 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/steveyegge/beads" target="_blank">
    


      
        steveyegge /

      beads</a></p>
				<p>
      Beads - A memory upgrade for your coding agent
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Go</span>
						<span>⭐ 13916</span>
						<span>🔱 834</span>
					</div>
				<div class="stars-today">⭐ 6841 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/BloopAI/vibe-kanban" target="_blank">
    


      
        BloopAI /

      vibe-kanban</a></p>
				<p>
      Get 10X more out of Claude Code, Codex or any coding agent
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Rust</span>
						<span>⭐ 19957</span>
						<span>🔱 1861</span>
					</div>
				<div class="stars-today">⭐ 11638 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/ComposioHQ/awesome-claude-skills" target="_blank">
    


      
        ComposioHQ /

      awesome-claude-skills</a></p>
				<p>
      A curated list of awesome Claude Skills, resources, and tools for customizing Claude AI workflows
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Python</span>
						<span>⭐ 28626</span>
						<span>🔱 2757</span>
					</div>
				<div class="stars-today">⭐ 15593 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/badlogic/pi-mono" target="_blank">
    


      
        badlogic /

      pi-mono</a></p>
				<p>
      AI agent toolkit: coding agent CLI, unified LLM API, TUI & web UI libraries, Slack bot, vLLM pods
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 TypeScript</span>
						<span>⭐ 4057</span>
						<span>🔱 463</span>
					</div>
				<div class="stars-today">⭐ 2823 stars this month</div>
				</div>
			</div>
	
			<div class="repo-card">
				<p><a href="https://github.com/DataTalksClub/data-engineering-zoomcamp" target="_blank">
    


      
        DataTalksClub /

      data-engineering-zoomcamp</a></p>
				<p>
      Data Engineering Zoomcamp is a free 9-week course on building production-ready data pipelines. The next cohort starts in January 2026. Join the course here 👇🏼
    </p>
				<div class="repo-stats">
					<div>
						<span>🔠 Jupyter Notebook</span>
						<span>⭐ 37725</span>
						<span>🔱 7632</span>
					</div>
				<div class="stars-today">⭐ 3633 stars this month</div>
				</div>
			</div>
	

		</div>
    </main>
{{< /raw >}}
