---
title: GitHub 趋势 2025年3月
date: 2025-03-20T07:58:50Z
categories:
- monthly
---
<link rel="stylesheet" href="public/css/trending.css">
{{< raw >}}
	<main class="container">
        <div class="repo-list" id="repoList">

	
			<div class="repo-card">
				<h2><a href="https://github.com/ValveSoftware/source-sdk-2013" target="_blank">
    


      
        ValveSoftware /

      source-sdk-2013</a></h2>
				<p>
      The 2013 edition of the Source SDK
    </p>
				<div class="repo-stats">
					<span>🔠 C++</span>
					<span>⭐ 8492</span>
					<span>🔱 2535</span>
				</div>
				<div class="stars-today">⭐ 3461 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/geekan/MetaGPT" target="_blank">
    


      
        geekan /

      MetaGPT</a></h2>
				<p>
      🌟 The Multi-Agent Framework: First AI Software Company, Towards Natural Language Programming
    </p>
				<div class="repo-stats">
					<span>🔠 Python</span>
					<span>⭐ 53007</span>
					<span>🔱 6274</span>
				</div>
				<div class="stars-today">⭐ 6631 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/LadybirdBrowser/ladybird" target="_blank">
    


      
        LadybirdBrowser /

      ladybird</a></h2>
				<p>
      Truly independent web browser
    </p>
				<div class="repo-stats">
					<span>🔠 C++</span>
					<span>⭐ 36091</span>
					<span>🔱 1507</span>
				</div>
				<div class="stars-today">⭐ 8118 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/langgenius/dify" target="_blank">
    


      
        langgenius /

      dify</a></h2>
				<p>
      Dify is an open-source LLM app development platform. Dify's intuitive interface combines AI workflow, RAG pipeline, agent capabilities, model management, observability features and more, letting you quickly go from prototype to production.
    </p>
				<div class="repo-stats">
					<span>🔠 TypeScript</span>
					<span>⭐ 83987</span>
					<span>🔱 12386</span>
				</div>
				<div class="stars-today">⭐ 14656 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/glanceapp/glance" target="_blank">
    


      
        glanceapp /

      glance</a></h2>
				<p>
      A self-hosted dashboard that puts all your feeds in one place
    </p>
				<div class="repo-stats">
					<span>🔠 Go</span>
					<span>⭐ 19159</span>
					<span>🔱 680</span>
				</div>
				<div class="stars-today">⭐ 8032 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/virattt/ai-hedge-fund" target="_blank">
    


      
        virattt /

      ai-hedge-fund</a></h2>
				<p>
      An AI Hedge Fund Team
    </p>
				<div class="repo-stats">
					<span>🔠 Python</span>
					<span>⭐ 18421</span>
					<span>🔱 3302</span>
				</div>
				<div class="stars-today">⭐ 9641 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/microsoft/OmniParser" target="_blank">
    


      
        microsoft /

      OmniParser</a></h2>
				<p>
      A simple screen parsing tool towards pure vision based GUI agent
    </p>
				<div class="repo-stats">
					<span>🔠 Jupyter Notebook</span>
					<span>⭐ 20834</span>
					<span>🔱 1700</span>
				</div>
				<div class="stars-today">⭐ 7820 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/langchain-ai/open_deep_research" target="_blank">
    


      
        langchain-ai /

      open_deep_research</a></h2>
				<p></p>
				<div class="repo-stats">
					<span>🔠 Jupyter Notebook</span>
					<span>⭐ 2348</span>
					<span>🔱 315</span>
				</div>
				<div class="stars-today">⭐ 1749 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/punkpeye/awesome-mcp-servers" target="_blank">
    


      
        punkpeye /

      awesome-mcp-servers</a></h2>
				<p>
      A collection of MCP servers.
    </p>
				<div class="repo-stats">
					<span>🔠 </span>
					<span>⭐ 12831</span>
					<span>🔱 834</span>
				</div>
				<div class="stars-today">⭐ 8947 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/codecrafters-io/build-your-own-x" target="_blank">
    


      
        codecrafters-io /

      build-your-own-x</a></h2>
				<p>
      Master programming by recreating your favorite technologies from scratch.
    </p>
				<div class="repo-stats">
					<span>🔠 Markdown</span>
					<span>⭐ 360373</span>
					<span>🔱 33453</span>
				</div>
				<div class="stars-today">⭐ 24035 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/sinaptik-ai/pandas-ai" target="_blank">
    


      
        sinaptik-ai /

      pandas-ai</a></h2>
				<p>
      Chat with your database or your datalake (SQL, CSV, parquet). PandasAI makes data analysis conversational using LLMs and RAG.
    </p>
				<div class="repo-stats">
					<span>🔠 Python</span>
					<span>⭐ 18249</span>
					<span>🔱 1698</span>
				</div>
				<div class="stars-today">⭐ 3754 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/jingyaogong/minimind" target="_blank">
    


      
        jingyaogong /

      minimind</a></h2>
				<p>
      🚀🚀 「大模型」2小时完全从0训练26M的小参数GPT！🌏 Train a 26M-parameter GPT from scratch in just 2h!
    </p>
				<div class="repo-stats">
					<span>🔠 Python</span>
					<span>⭐ 16539</span>
					<span>🔱 1827</span>
				</div>
				<div class="stars-today">⭐ 6122 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/gorhill/uBlock" target="_blank">
    


      
        gorhill /

      uBlock</a></h2>
				<p>
      uBlock Origin - An efficient blocker for Chromium and Firefox. Fast and lean.
    </p>
				<div class="repo-stats">
					<span>🔠 JavaScript</span>
					<span>⭐ 51776</span>
					<span>🔱 3391</span>
				</div>
				<div class="stars-today">⭐ 2150 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/svcvit/Awesome-Dify-Workflow" target="_blank">
    


      
        svcvit /

      Awesome-Dify-Workflow</a></h2>
				<p>
      分享一些好用的 Dify DSL 工作流程，自用、学习两相宜。 Sharing some Dify workflows.
    </p>
				<div class="repo-stats">
					<span>🔠 </span>
					<span>⭐ 4345</span>
					<span>🔱 462</span>
				</div>
				<div class="stars-today">⭐ 1397 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/78/xiaozhi-esp32" target="_blank">
    


      
        78 /

      xiaozhi-esp32</a></h2>
				<p>
      Build your own AI friend
    </p>
				<div class="repo-stats">
					<span>🔠 C++</span>
					<span>⭐ 9386</span>
					<span>🔱 1687</span>
				</div>
				<div class="stars-today">⭐ 4045 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/n8n-io/n8n" target="_blank">
    


      
        n8n-io /

      n8n</a></h2>
				<p>
      Fair-code workflow automation platform with native AI capabilities. Combine visual building with custom code, self-host or cloud, 400+ integrations.
    </p>
				<div class="repo-stats">
					<span>🔠 TypeScript</span>
					<span>⭐ 68913</span>
					<span>🔱 17655</span>
				</div>
				<div class="stars-today">⭐ 7204 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/huggingface/lerobot" target="_blank">
    


      
        huggingface /

      lerobot</a></h2>
				<p>
      🤗 LeRobot: Making AI for Robotics more accessible with end-to-end learning
    </p>
				<div class="repo-stats">
					<span>🔠 Python</span>
					<span>⭐ 10543</span>
					<span>🔱 1162</span>
				</div>
				<div class="stars-today">⭐ 1402 stars this month</div>
			</div>
	
			<div class="repo-card">
				<h2><a href="https://github.com/NirDiamant/GenAI_Agents" target="_blank">
    


      
        NirDiamant /

      GenAI_Agents</a></h2>
				<p>
      This repository provides tutorials and implementations for various Generative AI Agent techniques, from basic to advanced. It serves as a comprehensive guide for building intelligent, interactive AI systems.
    </p>
				<div class="repo-stats">
					<span>🔠 Jupyter Notebook</span>
					<span>⭐ 9171</span>
					<span>🔱 1184</span>
				</div>
				<div class="stars-today">⭐ 3119 stars this month</div>
			</div>
	

		</div>
    </main>
{{< /raw >}}
